...
public class VuforiaFindLocation{
    ...
    private double SkystoneOffsetY = 6.5; // This is the offset for the arm to camera. Was 7.5 before
    private double SkystoneOffsetX = 2;   // Offset of robot center to camera
    private double SkystoneScaleX = 0.9;  // Scale factor from trials
    private double SkystoneOffsetYP = 0;  // Y Offset specific to Position Case 1 ONLY
    ...
    public VuforiaFindLocation(LinearOpMode opmode){
        this.opmode = opmode;
        //this.hardwareMap = opmode.hardwareMap;
    }


    public double[] vuforia() {
        ...
        while (!targetVisible && runtime.seconds() <= 0.5) {
            ...
            if (targetVisible) {
                ...
                SkyStoneXYP[0] = translation.get(1)/mmPerInch;
                SkyStoneXYP[1] = translation.get(0)/mmPerInch;
                ...
                this.opmode.telemetry.addData("Position Y", positionY);
                this.opmode.telemetry.update();
        ...
        SkyStoneXYP[2] = positionCase;
        SkyStoneXYP[0] = SkystoneScaleX*SkyStoneXYP[0]+SkystoneOffsetX;  // Update X value with offset & scaling factor
        SkyStoneXYP[1] = -(SkyStoneXYP[1])-SkystoneOffsetY;

        // -------- This is Y tuning for Position Case - 1 ONLY ---------

        if (positionCase == 1) {
            SkyStoneXYP[1] = ((SkyStoneXYP[1]) - (SkystoneOffsetYP));
        }
